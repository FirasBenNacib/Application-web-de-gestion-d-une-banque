...
public class UtilisateurDetails implements UserDetails {
    ...
    public Collection<? extends GrantedAuthority> getAuthorities() {  //overriding getAuthorities method to work with authority scheme on the Data Base
        ...
        if (utilisateur.getRole() == 0) {  //role value can not be null in the database or we have problems.
    ...
    public boolean isEnabled(){ //option to enable/disable users required
        return utilisateur.getEnabled()!=0;//transforming from INT value to boolean
    }
}
